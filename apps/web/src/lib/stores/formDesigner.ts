  ...
  return {
    ...
    copyNode(nodeId: string) {
      update(state => {
        ...
        return {
          ...state,
          clipboard: cloneNode(nodeToCopy)
        };
      });
    },
    ...
    pasteNode(parentId: string) {
      update(state => {
        if (!state.form || !state.clipboard) return state;

        const newState = pushToUndo(state);
        const nodeToPaste = cloneNode(state.clipboard); // Clone again to allow multiple pastes
